...
| 表單驗證碼 | Google reCAPTCHA | 改由 Formspree 內建的垃圾訊息過濾負責 |
| 主機費用 | 需付費 PHP 主機 | **免費** |
...
| 防機器人 | Formspree 內建過濾（詳見下方「不要再加蜜罐欄位」） |
...
曾經加過一個隱藏的 `company_website` 欄位擋機器人，邏輯是「有值就判定為機器人，
安靜地不送出」。實測發現 **Chrome 的自動填入會連隱藏欄位一起填**
（`autocomplete="off"` 對 Chrome 沒有約束力），結果真人送出時每次都被誤判丟棄，
而且完全沒有任何提示，使用者只會覺得「按了沒反應」。花了很久才查出來。

擋掉真實客戶詢問的代價遠高於放過幾則垃圾訊息。若日後真的要自建防機器人機制：

- 不要用會被自動填入的 `text` 欄位
- **無論如何都不要「安靜地」丟棄送出**，一定要讓使用者看到發生了什麼
...
- **改到 `contact-form.js` 時，記得把 `contact/index.html` 裡的 `?v=3` 數字加 1。**
